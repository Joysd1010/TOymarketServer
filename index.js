...
async function run() {
  try {    
    ...
    const toyshop = client.db('ToysShop').collection('Catagory');
    const alltoys = client.db('ToysShop').collection('toys');
    const myToy = client.db('ToysShop').collection('mytoy');

    app.get('/category', async (req, res) => {
        const cursor = toyshop.find();
  ...
  app.post('/mytoys', async(req, res) => {
    const toy = req.body;
    console.log('new toy', toy);
    const result = await myToy.insertOne(toy);
    res.send(result); 
});
